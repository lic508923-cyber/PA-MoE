...
def load_model(path: str, device: torch.device, backbone_override: str | None = None,
    ...
    model = PAMoELog(
        ...
        expert_dora_enabled=bool(config.get("expert_dora_enabled", False)),
        dora_alpha=config.get("dora_alpha"),
        deep_dora_enabled=bool(config.get("deep_dora_enabled", False)),
        deep_dora_rank=int(config.get("deep_dora_rank", 8)),
        deep_dora_alpha=config.get("deep_dora_alpha"),
